...
var config = require('../CADViewer_config_ReactJS.json');
...
router.get('/getcadviewercontent', function (req, res) {
    ...
    var mynonce = crypto.randomBytes(16).toString("base64");
    var myCOP = config.ContentSecurityPolicy;
    myCOP = myCOP.replace('INSERTNONCE', mynonce);

    // 7.1.14
    if (config.ContentSecurityPolicyReportOnly)
        res.setHeader('Content-Security-Policy-Report-Only', myCOP);
    else
        res.setHeader('Content-Security-Policy', myCOP);
    ...
    try {
        ...
        if (returnCadviewer == 1) {   // cadviewer response
            ...
            var orgFile = config.ServerLocation + "/html/CADViewer_base_7_restresponse.html";
            ...
            fs.copyFile(orgFile, writeFile, (err) => {
                if (err) {
                    ...
                    fs.readFile(writeFile, 'utf8', function (err, data) {
                        ...
                        // 7.1.15 -insertnonce
                        mystring0 = mystring0.replace('INSERTNONCE', mynonce);
